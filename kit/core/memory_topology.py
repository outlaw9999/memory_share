...
class MemoryTopology:
    ...
    def __init__(self, project_root: Path | None = None):
        ...
        # v1.2.5: Cache home resolution to avoid repeated OS overhead
    ...
    def describe_topology(self) -> dict:
        ...
        def _portable_path(p: Path | None) -> str:
            if not p:
                return "none"
            try:
                # v1.2.5: Prefer relative or home-based paths for portability
    ...
    def connect(
        ...
        # 1.2.5ARCHITECTURE-LOCK: Default to RO for Frozen/Snapshot, but allow override
        if readonly is None:
            is_readonly = is_frozen or is_snapshot
        else:
            is_readonly = readonly

        # v1.2.5: Unified URI construction for Windows compatibility
    ...
    def connect_path(self, path: Path, readonly: bool = False, timeout: float = 10.0) -> sqlite3.Connection:
        """
        1.2.5ARCHITECTURE-LOCK: Authorized method to connect to arbitrary paths.
