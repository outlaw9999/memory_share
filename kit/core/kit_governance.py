...
def run_preflight(
    ...
    # 1. Fetch staged changes, or use explicit piped content for agent/runtime validation.
    if diff_text is None:
        try:
            diff_output = subprocess.check_output(
                ["git", "diff", "--cached", "--unified=0"], stderr=subprocess.STDOUT, text=True, errors="replace"
            )
            staged_files = subprocess.check_output(
                ["git", "diff", "--cached", "--name-only"], text=True, errors="replace"
            ).splitlines()
    ...
    if diff_text is None:
        loc_changed = len(
            [line for line in diff_output.splitlines() if line.startswith("+") and not line.startswith("+++")]
        )
    else:
        loc_changed = len([line for line in diff_output.splitlines() if line.strip()])
    ...
    sem_rows = brain.get_semantic_observations(limit=limit)
