...
class BrainMigrationSkill(BaseSkill[BrainMigrationInput]):
    ...
    def run(self, input_data: BrainMigrationInput, context: list = None) -> SkillOutput:
        ...
        try:
            migrate_and_merge(input_data.old_db_path, input_data.new_db_path, input_data.output_db_path)
            return SkillOutput(
                status="SUCCESS",
...
def migrate_and_merge(old_db_path, new_db_path, output_db_path):
    ...
    try:
        ...
        for record in source_records:
            ...
            for col in target_cols:
                if col in source_cols:
                    ...
                    if col == "is_baked":
                        ...
                        new_data[col] = 1
                    elif col == "is_canonical":
                        new_data[col] = 0
                    elif col == "symbol_locked":
                        new_data[col] = 0
                    elif col == "symbol_confidence":
                        new_data[col] = 1.0
                    elif col == "symbol_source":
                        new_data[col] = "migration"
                    elif col == "id":
                        new_data[col] = None  # Let SQLite auto-increment
                    else:
                        new_data[col] = None
            ...
            target_conn.execute(
                f"INSERT INTO observations ({cols_str}) VALUES ({placeholders})", list(new_data.values())
            )
            new_inserts += 1
