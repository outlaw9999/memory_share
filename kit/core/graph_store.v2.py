...
class GraphStore:
    ...
    def _init_schema(self) -> None:
        ...
        # Covering indices
        cur.execute("CREATE INDEX IF NOT EXISTS idx_edges_source_layer ON edges(source_id, layer)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_edges_target_layer ON edges(target_id, layer)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_nodes_symbol ON nodes(symbol)")
        
        self.conn.commit()

    def get_or_create_node(self, symbol_name: str, kind: Optional[str] = None, file_path: Optional[str] = None, line_number: Optional[int] = None) -> int:
    ...
    def add_edge(self, source_symbol: str, target_symbol: str, layer: int = 0, weight: float = 1.0):
    ...
    def update_file(
        ...
        try:
            ...
            if file_node_ids:
                ...
                cur.execute(f"DELETE FROM edges WHERE source_id IN ({placeholders})", file_node_ids)
