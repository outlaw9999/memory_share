...
class TransactionManager:
    ...
    def start_transaction(self, flow_id: str, step_id: str, attempt: int) -> FlowExecutionContext:
        ...
        return FlowExecutionContext(flow_id=flow_id, step_id=step_id, transaction_id=transaction_id, attempt=attempt)
...
class FlowCommitController:
    ...
    def finalize(self, flow_id: str):
        ...
        with self.brain.get_connection() as conn:
            ...
            conn.execute(
                "UPDATE flow_runs SET state = 'success', updated_at = CURRENT_TIMESTAMP WHERE id = ?", (flow_id,)
            )
...
class FlowExecutor:
    ...
    def execute(self, spec: FlowSpec) -> bool:
        ...
        with self.brain.get_connection() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO flow_runs (id, name, state) VALUES (?, ?, 'running')", (spec.id, spec.name)
            )
        ...
        for step in ordered_steps:
            ...
            if step.idempotent:
                with self.brain.get_connection() as conn:
                    row = conn.execute(
                        "SELECT state FROM flow_steps WHERE id = ?", (f"{spec.id}:{step.id}",)
                    ).fetchone()
            ...
            frame = ExecutionFrame(command=reified_cmd, context=ctx, max_retries=step.retry)
