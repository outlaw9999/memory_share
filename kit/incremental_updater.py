...
class IncrementalUpdater:
    ...
    def _compute_symbol_delta(
        ...
        for name, new_sym in new_symbol_map.items():
            ...
            if name not in old_symbols:
                ...
                old_hash = self._compute_symbol_hash(
                    old_symbols[name],
                    file_path
                )
    ...
    def _delete_symbols_in_set(self, symbols: List[Dict]) -> None:
        ...
        cur.execute(
            f"DELETE FROM symbols WHERE name IN ({placeholders})",
            names
        )

    def _insert_symbols(self, symbols: List[Dict]) -> None:
        """Insert new symbols."""
        if not symbols:
            return
    ...
    def _insert_edges(self, edges: List[Dict]) -> None:
    ...
    def get_file_change_impact(self, file_path: str) -> Dict[str, Any]:
        ...
        # Symbols in file
        cur.execute(
            "SELECT COUNT(*) as cnt FROM symbols WHERE file = ?",
            (file_path,)
        )
