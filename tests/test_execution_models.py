...
def test_high_confidence_enforced():
    ...
    run_kit(
        [
            ...
            "db",
            "--content",
            "All database operations MUST use SQLite. Do NOT mention PostgreSQL.",
            "--symbol",
            "db_system",
        ]
    )
...
def test_ambiguous_behavior():
    ...
    run_kit(
        ["learn", "--tag", "decision", "--uid", "cache", "--content", "Use Redis for caching.", "--symbol", "caching"]
    )
    run_kit(
        [
            "learn",
...
def test_weak_signal_flexible():
    ...
    run_kit(
        [
            ...
            "logging_note",
            "--content",
            "Maybe consider file-based logging.",
            "--symbol",
            "logging",
        ]
    )
    ...
    # 3. Verify Flexibility (check decision is not BLOCK)
    lower_out = output.lower()
    # Accept either WEAK SIGNAL behavior or the default flexible behavior
    assert "warn" in lower_out or "pass" in lower_out
