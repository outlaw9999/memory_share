...
class CodebaseIngestor:
    ...
    def ingest_ast(self, tree, module_path, rel_path):
        ...
        api.learn(module_path, "module", f"Python module located at {rel_path}", layer="semantic")
        
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                class_uid = f"{module_path}.{node.name}"
                api.learn(class_uid, "class", f"Class {node.name} defined in {module_path}", layer="semantic")
                api.link_entities(module_path, class_uid, "CONTAINS")
                
                # Extract Docstring
                doc = ast.get_docstring(node)
                if doc:
                    api.learn(class_uid, "arch", doc.strip(), layer="semantic", importance=0.8)
                    
            elif isinstance(node, ast.FunctionDef):
                func_uid = f"{module_path}.{node.name}"
                api.learn(func_uid, "function", f"Function {node.name} in {module_path}", layer="semantic")
                api.link_entities(module_path, func_uid, "CONTAINS")
                
                doc = ast.get_docstring(node)
                if doc:
                    api.learn(func_uid, "arch", doc.strip(), layer="semantic", importance=0.7)

            elif isinstance(node, ast.ImportFrom):
                if node.module:
                    # Ensure target exists as a placeholder
                    api.learn(node.module, "module", f"External or unindexed module: {node.module}", layer="semantic")
                    api.link_entities(module_path, node.module, "DEPENDS_ON")
            
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    api.learn(alias.name, "module", f"Imported module: {alias.name}", layer="semantic")
                    api.link_entities(module_path, alias.name, "DEPENDS_ON")
    ...
    def ingest_heuristics(self, content, module_path, rel_path):
        ...
        for i, line in enumerate(lines):
            ...
            if todo_match:
                ...
                api.learn(module_path, "task", f"Line {i+1}: {task_content}", layer="working", importance=0.9)
            ...
            if proc_match:
                ...
                api.learn(module_path, "procedural", proc_content, layer="procedural", importance=0.6)
