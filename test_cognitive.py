...
def cognitive_layers_test():
    ...
    api.learn("coding_style", "Always use 4 spaces for indentation", kind="procedural", layer="procedural")
    
    # Semantic: A definition from a month ago
    api.learn("jwt", "JWT stands for JSON Web Token", kind="semantic", layer="semantic")
    
    # Episodic: A recent event
    api.learn("deploy", "Deployed version 1.1.1 successfully", kind="episodic", layer="episodic")
    
    # Working: A very fresh task
    api.learn("debug", "Investigating Redis timeout", kind="task", layer="working")
    
    # Simulate time passing via manual DB update
    with api.get_brain()._get_connection() as conn:
        # Instruction is 1 year old
        conn.execute("UPDATE observations SET created_at = julianday('now', '-365 days') WHERE content LIKE '%spaces%'")
        # Definition is 60 days old
        conn.execute("UPDATE observations SET created_at = julianday('now', '-60 days') WHERE content LIKE '%JSON%'")
        # Working task is 3 hours old
        conn.execute("UPDATE observations SET created_at = julianday('now', '-0.125 days') WHERE content LIKE '%Redis%'")
    ...
    for i, r in enumerate(results):
        print(f"  {i+1}. [{r.node_uid}] {r.content}")
