...
class GroundingEngine:
    ...
    def detect_intent(self, query: str) -> str:
        ...
        q = query.lower()
        if any(
            w in q for w in ["fail", "error", "bug", "wrong", "fix", "issue", "crash"]
        ):
            return "DEBUG"
        if any(
            w in q
            for w in ["depend", "call", "import", "architecture", "structure", "layer"]
        ):
            return "ARCHITECTURE"
        if any(
            w in q
            for w in ["why", "design", "decision", "chose", "history", "rationale"]
        ):
            return "DECISION"
        return "GENERAL"
    ...
    def rank_symbols(self, keyword: str, candidates: List[dict[str, Any]]) -> List[int]:
        """
