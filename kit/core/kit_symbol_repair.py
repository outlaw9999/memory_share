"""Titanium Symbol Repair Engine (1.2.5STAGE5.3).
...
def _repair_db(conn: sqlite3.Connection, label: str) -> int:
    ...
    try:
        ...
        for row in rows:
            ...
            # 1.2.5STAGE5.5: Record ambiguity sensor even if locked
        ...
        if repaired > 0 or migrated > 0:
            conn.commit()
            logger.info(
                f"Symbol Governance [{label}]: {repaired} repaired, {migrated} migrated, {skipped_protected} protected, {skipped_locked} locked."
            )
    except Exception as e:
