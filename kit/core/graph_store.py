...
class GraphStore:
    ...
    def add_symbol(self, fqn, kind=None, file_path=None):
        cur = self.conn.cursor()
    ...
    def add_alias(self, alias, symbol_id, confidence=1.0):
    ...
    def add_edge_by_alias(self, source_id, target_alias, layer=0):
        cur = self.conn.cursor()

        cur.execute("""
        INSERT INTO edges(source_id, target_alias, layer)
        VALUES (?, ?, ?)
        """, (source_id, target_alias.lower(), layer))
