...
GLOBAL_DB_PATH = Path.home() / ".kit" / "global.db"
if os.getenv("KIT_HOME"):
    GLOBAL_DB_PATH = Path(os.getenv("KIT_HOME")).expanduser() / "global.db"

TELEMETRY_PATH = Path.home() / ".kit" / "routing_telemetry.jsonl"
if os.getenv("KIT_HOME"):
    TELEMETRY_PATH = Path(os.getenv("KIT_HOME")).expanduser() / "routing_telemetry.jsonl"
...
def has_code(text: str) -> bool:
    return bool(
        "`" in text or
        re.search(r"\w+\.\w+", text) or
        re.search(r"\b[a-z]+[A-Z][a-z]+\b", text) or 
        re.search(r"\b[a-z]+_[a-z]+\b", text)
    )
...
def score(text: str) -> Dict[str, float]:
    ...
    return {
        "generality": min(g, 1.0),
        "specificity": min(s, 1.0),
        "reusability": min(r, 1.0)
    }
...
def route_content(text: str) -> Dict[str, Any]:
    ...
    if blocked:
        log({"status": "BLOCKED", "reason": reason, "ts": datetime.now(timezone.utc).isoformat()})
