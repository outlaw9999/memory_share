...
def decide(signals: list) -> dict:
    ...
    if has_high:
        return {"action": Action.BLOCK, "exit_code": 1, "reason": "High-confidence architectural signals detected."}

    # Medium/Low confidence -> WARN
    has_medium_low = any(get_conf(s) in ("medium", "low") for s in signals)
    if has_medium_low:
        return {"action": Action.WARN, "exit_code": 0, "reason": "Low-confidence smells detected. Review recommended."}
