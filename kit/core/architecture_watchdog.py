...
class ArchitecturePolicy:
    ...
    layers: List[str] = None  # e.g., ["api", "service", "repo", "util"]
...
class ArchitectureWatchdog:
    ...
    def _get_modified_symbols(self, changed_files: List[str]) -> List[str]:
        ...
        for file_path in changed_files:
            cur.execute(
                "SELECT DISTINCT name FROM symbols WHERE file = ?",
                (file_path,)
            )
    ...
    def _get_symbol_file(self, symbol: str) -> str:
        ...
        cur = self.conn.cursor()
        cur.execute(
            "SELECT file FROM symbols WHERE name = ? LIMIT 1",
            (symbol,)
        )
        row = cur.fetchone()
    ...
    def to_json(self) -> str:
        ...
        } for v in self.violations], indent=2)
...
def create_watchdog_middleware(watchdog_instance):
