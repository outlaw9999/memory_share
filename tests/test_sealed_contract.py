...
from kit.core.kit_sealing import SEALED_VERSION, verify_kernel_seal
from kit.core.memory_router import MemoryRouter, MemoryTier, MemoryWriteRequest, WriteSource
...
def test_memory_dto_immutability():
    ...
    m = Memory(id=1, node_uid="test", content="content", score=1.0, brain_source="local", metadata={"initial": True})
...
def test_memory_to_dict_mapping():
    ...
    m = Memory(
        id=42, node_uid="uid_42", content="hello", score=0.9, brain_source="global", tag="pattern", layer="semantic"
    )
    d = m.to_dict()
    assert d["uid"] == "uid_42"
